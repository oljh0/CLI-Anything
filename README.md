...
- **MCP Server** — 17 个工具供 AI Agent（Copilot CLI / Claude / GPT）直接调用
...
```bash
# 主终端
task register --name "主控台" --role master

# 子终端（另一个窗口）
task register --name "开发-1" --role worker
```
...
```bash
# 创建主任务
task create "实现用户认证" --desc "JWT + bcrypt" --priority 1 --tags "auth,security"

# 创建需要审阅的任务（可选）
task create "敏感接口重构" --desc "需审核后执行" --review

# 拆解为子任务
task decompose <task-id> --subtasks '[{"title":"登录接口"},{"title":"注册接口"},{"title":"Token刷新"}]'
...
```bash
# 审阅通过 → 任务进入 pending
task review <task-id> --approve --comment "方案可行"

# 审阅驳回 → 保持 draft，可重新提交
task review <task-id> --reject --comment "需要补充方案"

# 重新提交审阅
task resubmit-review <task-id>
```
...
# 领取
task claim <subtask-id>

# 开始工作
task start <subtask-id>

# 提交（自动运行测试）
task submit <subtask-id>
```
...
```bash
# 验收通过
task verify <subtask-id> --approve --comment "LGTM"

# 驳回
task verify <subtask-id> --reject --comment "缺少边界测试"
```
...
├── mcp_server/server.py       # MCP Server（17 个工具）
├── web/dashboard.py           # Web Dashboard（含审阅 REST API）
├── tui/app.py                 # TUI 终端界面（含审阅列）
├── notification/notifier.py   # 跨平台通知（已实现，待集成）
└── utils/
...
| 04 | [CLI 命令行](docs/04-CLI命令行.md) | 26 个命令详解、参数表、输出格式、使用示例 |
| 05 | [MCP Server](docs/05-MCP-Server.md) | 17 个工具详解、参数/返回格式、AI Agent 集成 |
